...
def test_worker_list_page_is_titled_and_not_part_of_leads(admin_client):
    ...
    assert '<span class="brand-mark" aria-hidden="true"></span>Worker list</a>' in html
    assert 'app.js' not in html and 'bottom-nav' not in html


def test_worker_list_is_only_in_the_admin_menu(client, admin_client):
    assert 'Worker list' not in client.get("/").get_data(as_text=True)
    assert 'Worker list' in admin_client.get("/").get_data(as_text=True)
